...
def predict(batch_size=1,conditioning='unaligned_phonemes'):
    itr = vctk_loader.data_loader('test',batch_size,conditioning=conditioning)
    count=0
    ...
    for i in tqdm(xrange(batch_size)):
        ...
        try:
            ...
            end_X = -1
        if conditioning=='unaligned_phonemes':
            pred_X = predict_fn(
