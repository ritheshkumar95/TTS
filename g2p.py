    ...
    for ch,ch_mask,ph,ph_mask in valid_itr:
        ...
        for i in xrange(batch_size):
            ...
            for ph in phs:
                ...
                tmp_wer += [wer(ph,hyp)]
                if len(ph)==len(hyp) and (np.asarray(hyp)==np.asarray(ph)).all():
                    count_exact_match += 1
            valid_costs.append(min(tmp_wer))

    print "Validation Completed! PER: {} WER: {} time: {}".format(np.mean(valid_costs),100-100*(count_exact_match/(iteration*16.)),time.time()-start)
    return np.mean(valid_costs)
